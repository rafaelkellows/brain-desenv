<?php
	...
	if(isset($_FILES["file"]["type"])){
		$validextensions = array("jpeg", "jpg", "png", "zip","pdf");
		$temporary = explode(".", $_FILES["file"]["name"]);
		$file_extension = end($temporary);
		if ((($_FILES["file"]["type"] == "image/png") || ($_FILES["file"]["type"] == "image/jpg") || ($_FILES["file"]["type"] == "image/jpeg") || ($_FILES["file"]["type"] == "application/x-zip-compressed") || ($_FILES["file"]["type"] == "application/pdf")
		...
		if ($_FILES["file"]["error"] > 0){
			...
			if (file_exists("upload/" . $_FILES["file"]["name"])) {
				...
				if ( move_uploaded_file($sourcePath,$uploadPath.$NewFileName) ) {
