          ...
          if($type == 'a'){
            if(!isset($_POST['usr_send'])){
              ...
              if($sqlInsert){
                ...
                $row_select = mysql_fetch_array($sqlSct);
                if($row_select){
                  //Get last id inserted 'tbl_uploads'
                  $id = $row_select['id'];
                  foreach ($_POST['usr_send'] as $users){
                    $sqlSct = $oConn->SQLselector("*","tbl_usr_upload","id_user = '".$users."' and id_upload = '".$id."'","");
                    $row_select = mysql_fetch_array($sqlSct);
                    if(!$row_select){
            ...
            foreach ($_POST['usr_send'] as $users){
              ...
              if(!$row_select){
                ...
                if($rUser) {
                    $row = mysql_fetch_array($oSlctUser);
                    if($row){
                      emailSend('newpost',$row['name'],$row['email'],'',$id,'resend',$complement);
                    }
                }else{
                   // header('location: ../pt_br/?msg=3');
                }
