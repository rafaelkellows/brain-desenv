...
jQuery(document).ready(function(){
    ...
    jQuery('.smh_theme_select').live('change', function(){
...
var sOptions = {
    ...
    update: function(event, ui) {
        var $selected_iconset = jQuery(this).closest('.widget-content').find('.smh_theme_select').val();
        var sortDiv = jQuery(this);
        var sortedIDs = sortDiv.sortable( "toArray" );
        jQuery.each(sortedIDs, function(index, value){
            sortDiv.find('DIV#'+value+' .smh_sort').val(index);
        });
        var oSortedIDs = toObject(sortedIDs);
        var $smh_sortable_data_raw = unescape(sortDiv.find('.smh_sortable').val());
        if ($smh_sortable_data_raw.length > 2){
            var $smh_sortable_data = jQuery.parseJSON($smh_sortable_data_raw);
            $smh_sortable_data[''+$selected_iconset+''] = oSortedIDs;
            var $serialized = escape(JSON.stringify($smh_sortable_data));
        } else {
            var $serialized = escape(JSON.stringify({$selected_iconset : oSortedIDs}));
        }
        sortDiv.find('.smh_sortable').val($serialized);
    },
    create: function( event, ui ) {
        var $selected_iconset = jQuery(this).closest('.widget-content').find('.smh_theme_select').val();
        var sortDiv = jQuery(this);
        var $serialized = unescape(sortDiv.find('.smh_sortable').val(), 'ENT_QUOTES');
        if ($serialized.length > 1){
            ...
            if ($unserialized_data[''+$selected_iconset+''] !== undefined) {
                ...
                jQuery('<div id="smh_sorted" class="iconset_container_bdr"></div>').insertAfter(sortDiv);
                ...
                jQuery('#smh_sorted DIV.icon_container').appendTo(sortDiv);
                jQuery('#smh_sorted').remove();                   
            }
        }                                             
    },
